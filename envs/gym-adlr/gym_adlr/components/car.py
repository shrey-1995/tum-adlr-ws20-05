...
import math
import Box2D
from Box2D import b2EdgeShape, b2CircleShape, b2FixtureDef, b2PolygonShape, b2RevoluteJointDef, b2ContactListener, b2Shape
...
class Car:
    def __init__(self, world, init_angle, init_x, init_y):
        ...
        self.hull = self.world.CreateDynamicBody(
            ...
            angle=init_angle,
            fixtures=[
                b2FixtureDef(shape=b2PolygonShape(vertices=[(x*SIZE, y*SIZE) for x, y in HULL_POLY1]), density=1.0),
                b2FixtureDef(shape=b2PolygonShape(vertices=[(x*SIZE, y*SIZE) for x, y in HULL_POLY2]), density=1.0),
                b2FixtureDef(shape=b2PolygonShape(vertices=[(x*SIZE, y*SIZE) for x, y in HULL_POLY3]), density=1.0),
                b2FixtureDef(shape=b2PolygonShape(vertices=[(x*SIZE, y*SIZE) for x, y in HULL_POLY4]), density=1.0)
                ]
            )
        ...
        for wx, wy in WHEELPOS:
            ...
            w = self.world.CreateDynamicBody(
                ...
                fixtures=b2FixtureDef(
                    shape=b2PolygonShape(vertices=[(x*front_k*SIZE,y*front_k*SIZE) for x, y in WHEEL_POLY]),
                    density=0.1,
                    categoryBits=0x0020,
                    maskBits=0x001,
                    restitution=0.0)
