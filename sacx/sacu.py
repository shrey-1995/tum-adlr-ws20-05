    ...
    def actor(self):
        ...
        for t in range(T):                          # Repeat for T time steps
            if t % xi == 0:                         # Check if a new task should be scheduled
                task = self.schedule_task()      # If so, sample a new task from the scheduler
                Tau.append(task)
                print("Switching to ", task)
                h += 1                              # Update number of tasks scheduled

            a = task.get_action(s, 0)               # Sample action according to latest task

            next_state, reward, done, _ = self.env.step(a)              # Execute action, obtain observation and rewards
            tau.append((s, a, reward))            # Add to trajectory
            s = next_state
            score += np.array(t)
            if done:
                break
