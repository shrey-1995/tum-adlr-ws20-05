...
class SACXAgent():
    def __init__(self,
        ...
        self.p_nets = self.init_p_models()
        self.q_nets1, self.q_nets2, self.target_q_nets1, self.target_q_nets2 = self.init_q_models(tasks)
        self.q1_optimizers, self.q2_optimizers, self.policy_optimizers = self.init_optimizers(q_lr, p_lr)
        self.entropy_temperatures = self.init_temperatures(a_lr, alpha)

        self.replay_buffer = BasicBuffer(buffer_maxlen)

    def init_p_models(self):
        policy_nets = []
        for i, task in enumerate(self.tasks):
            policy_nets.append(PolicyNetwork(self.obs_dim, self.action_dim).to(self.device))
        return policy_nets

    def init_q_models(self, tasks):
        q_nets1 = []
        q_nets2 = []
        target_q_nets1 = []
        target_q_nets2 = []
    ...
    def init_optimizers(self, q_lr, policy_lr):
        q1_optimizers = []
        q2_optimizers = []
        policy_optimizers = []
    ...
    def init_temperatures(self, a_lr, alpha):
        temperatures = []
